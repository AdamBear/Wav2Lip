...
parser.add_argument('--pads', nargs='+', type=int, default=[0, 10, 0, 0],
                    help='Padding (top, bottom, left, right). Please adjust to include chin at least')  # default=[0, 10, 0, 0] = caffe [-15 5 0 0]
...
parser.add_argument('--rotate', default=True, action='store_true',
...
parser.add_argument('--no_rotation', default=False, action='store_true',
                    help='Shows preview frame to horizontically align the face in the video')

parser.add_argument('--pingpong', default=False, action='store_true',
                    help='Loop video backwards/forward on longer audio')

parser.add_argument('--no_caffe', default=False, action='store_true', help='don not use caffemodel for face detection')

parser.add_argument('--img_sequence', default=False, action='store_true',
                    help='Save result as image sequence in png format')

parser.add_argument('--face_sequence', default=False, action='store_true',
                    help='use image sqeuence not video that contains the face to use')
...
def main():
    ...
    # - mouth
    src_mask = np.zeros((192, 192), dtype=np.uint8)
    ...
    if not os.path.isfile(args.face) and not args.face_sequence:
        ...
        if args.outfile == 'temp/silence.mp4':
            command = 'ffmpeg -y -f lavfi -i anullsrc -t ' + str(
                duration) + ' -acodec libmp3lame -ac 2 -ar 44100 -ab 128000 temp/silence.mp3'
        ...
        while 1:
            ...
            if args.resize_factor != 1.0:
                frame = cv2.resize(frame, (int(frame.shape[1] // args.resize_factor), int(frame.shape[0] // args.resize_factor)))
    ...
    for i, (img_batch, mel_batch, frames, coords) in enumerate(
        ...
        for p, f, c in zip(pred, frames, coords):
            ...
            if 1 < n_frames < framecount and not loop:
                if args.outfile == 'temp/silence.mp4':
                    sys.exit()
                out.release()
